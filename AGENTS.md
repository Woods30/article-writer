...
- **No automated test suite exists.** Validation is done via `validate` CLI command and the `validate_bundle.py` script.
- **No linter configuration** (no `pyproject.toml`, `setup.cfg`, `ruff.toml`, or `.flake8`). Code style checks are not enforced.
- **Live execution** (without `--dry-run`) requires `OPENAI_API_KEY` environment variable. Without it, only dry-run and validation commands work.
- **`googleSearch`, `fetch`, `runSkill`, `searchApi` tools** referenced in agent manifests are not implemented in `agno_runtime/tools.py` (only `now_utc`, `read`, `write`, `edit` are built-in). Agents referencing missing tools will have those tools silently skipped at build time.
- The `.cursor/environment.json` file has been deleted from the repo so that snapshot-managed environment settings take effect. The update script (`pip install -r requirements-agno.txt`) handles dependency installation.
